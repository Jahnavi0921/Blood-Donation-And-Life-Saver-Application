...
def AddHspAction(request):
    ...
    print(d)
    if int(d[0]) > 0:
        context = {'msg': 'BloodBank Name Already Exist...!!'}
        ...
        context = {'msg': 'BloodBank Name Added Successfully...!! AND Login Credentials Sent to Mail'}
        return render(request, 'AdminApp/AddHsp.html', context)
...
def ViewHsp(request):
    ...
    dd = cur.fetchall()
    table_data="<table>"
    table_data+="<tr><th>BloodBank Name</th>"
    table_data+="<th>BloodBank Contact</th>"
    table_data += "<th>BloodBank Email</th>"
    table_data += "<th>BloodBank Address</th>"
...
def SearchAction(request):
    ...
    location1 = geolocator.geocode(c_loc)
    ...
    for d in dd:
        ...
        for dd in ddd:
            bb_address = d[4] + ",India"
            location2 = geolocator.geocode(bb_address)
            ...
            table_data += "<tr><td>" + d[1] + "</td>"
            table_data += "<td>" + bb_address + "</td>"
            table_data += "<td>" + dd[3] + "</td>"
            table_data += "<td>" + dd[7] + "</td>"
            table_data += "<td>" + dd[5] + "</td>"
            table_data += f"<td>{distance_km:.2f} Km</td>"
...
def RequestAction(request):
    ...
    con.commit()
    if i > 0:
        context = {'msg': 'Request Sent To BloodBank Successful..!!'}
        return render(request, 'RequestBlood.html', context)
    else:
...
def ViewFeedback(request):
    ...
    table_data = "<table>"
    table_data += "<tr><th>BloodBank ID</th>"
